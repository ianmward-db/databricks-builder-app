...
def init_database(database_url: Optional[str] = None) -> AsyncEngine:
    ...
    if url:
        ...
        # For service principals, user_name is None - fall back to DATABRICKS_CLIENT_ID (the SP's applicationId,
        # which matches the token's 'sub' claim generated by generate_database_credential)
        username = (
            os.environ.get("LAKEBASE_USERNAME")
            or _get_current_user_email()
            or os.environ.get("DATABRICKS_CLIENT_ID")  # SP application ID in Databricks Apps
            or _lakebase_instance_name
        )
...
def is_postgres_configured() -> bool:
    ...
    return bool(
        ...
        or (
            (os.environ.get("LAKEBASE_INSTANCE_NAME") or os.environ.get("LAKEBASE_ENDPOINT"))
...
def is_dynamic_token_mode() -> bool:
    ...
    return bool(
        ...
        and (os.environ.get("LAKEBASE_INSTANCE_NAME") or os.environ.get("LAKEBASE_ENDPOINT"))
        and os.environ.get("LAKEBASE_DATABASE_NAME")
    )
